...
export function validateLogin(value: string): string | null {
  ...
  if (!/^[a-zA-Z0-9_-]+$/.test(value)) return "Только латиница, цифры, дефис и нижнее подчёркивание";
  return null;
}

export function validateName(value: string): string | null {
  if (!value) return "Обязательное поле";
  if (!/^[A-ZА-ЯЁ][a-zA-Zа-яёА-ЯЁ-]*$/.test(value)) return "Только буквы, первая — заглавная";
  return null;
}

export function validatePhone(value: string): string | null {
  if (!value) return "Обязательное поле";
  if (!/^\+?\d{10,15}$/.test(value)) return "От 10 до 15 цифр, допускается знак +";
  return null;
}
